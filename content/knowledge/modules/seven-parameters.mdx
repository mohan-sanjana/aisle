Parts I and II built up the vocabulary. You know what inference is, how a token gets made, why HBM bandwidth and the KV cache are the dominant constraints, and why traditional planning assumptions break for AI workloads. This module turns that vocabulary into a short list of inputs the Sizer (and every infrastructure conversation) takes. Seven parameters. Get them right and the sizing exercise becomes mechanical. Get them wrong, or skip one, and you spend weeks rebuilding the spreadsheet later.

The Sizer takes about twenty inputs in total, but these seven are the load-bearing ones. The rest either follow from these or default sensibly.
...
**What it is.** The specific model the AI team has chosen. Two attributes of that choice drive sizing: the **parameter count** (in billions, e.g., 7B, 70B, 405B) and the **architecture** (dense, grouped-query attention, mixture-of-experts).

**How it moves the math.** Parameter count sets the floor on GPU memory. At FP8, each parameter takes one byte, so a 70B model needs 70 GB just to hold the weights, before any conversation memory or activations. The architecture determines how the model uses memory beyond that. Dense models read every parameter on every token. MoE models activate only a fraction per token but still need all parameters resident in HBM, which changes the bandwidth math substantially. The model has to be re-read on every output token, so the choice of model is the single largest contributor to required memory bandwidth.

**What to ask.** "Which model exactly?" not "how big a model?" A 70B dense model and a 70B-class MoE model have very different infrastructure profiles even though the headline number is the same. The Sizer lets you pick a model family (Llama 3.1 70B, GPT-OSS 120B, etc.) which fills in both attributes from a canonical spec, or roll your own by entering parameter count and architecture separately.
...
**How it moves the math.** Context drives the <GlossaryTerm id="kv_cache">KV cache</GlossaryTerm>, which is the per-request working memory you met in the KV cache module. KV cache size is linear in context length. Double the context, double the KV memory per request. The Sizer uses average length for the steady-state budget and treats `max_context` as a guardrail for worst-case bursts.
...
**What it is.** The number of distinct callers talking to the system at the same time during peak hours. In a chat application, this is the number of human users actively in a conversation. In an agentic workload, every agent and every sub-agent that holds an open LLM session also counts. A single human kicking off a research agent that spawns ten parallel sub-agents drives ten concurrent in-flight requests, not one. This matters more every quarter as agentic systems become the dominant production pattern.
...
**What to ask.** "Peak concurrent callers during the busiest hour, counting every agent invocation separately." A SaaS application with 50,000 registered users might have 200 concurrent humans at peak. Plan against 200, not 50,000. For agentic systems, multiply that by the typical fan-out of agents per human action.
